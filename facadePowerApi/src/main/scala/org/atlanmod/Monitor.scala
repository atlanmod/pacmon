...
import org.powerapi.{PowerDisplay, PowerMeter, PowerModule, PowerMonitoring}
...
class Monitor {
  ...
  var module: PowerModule = _

  def run(pid: Int): Unit = {
    if (module == null) {
      module = new CpuSimpleModule(new LinuxHelper, tdp, tdpFactor)
    }
